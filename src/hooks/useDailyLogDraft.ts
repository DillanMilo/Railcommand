...
import { registerPendingDraftSave } from '@/lib/offline/pending-draft-saves';
...
export function useDailyLogDraft({
  ...
  useEffect(() => {
    ...
    void readDailyLogDraft(offlineUserId, projectId)
      ...
      .catch(() => {
        ...
        // A failed read must not authorize an overwrite of an unread draft.
        // Sign-out's strict flush will fail closed while hydration is unsafe.
      });
  ...
  const persistLatest = useCallback(async (requireSaved = false) => {
    if (!enabled || !offlineUserId || !projectId) return;
    if (!hydratedRef.current) {
      if (requireSaved) throw new Error('The saved draft could not be verified. Keep this page open and try again.');
      return;
    }
  ...
  useEffect(() => {
    if (!enabled || !offlineUserId || !projectId) return;
    return registerPendingDraftSave(offlineUserId, () => persistLatest(true));
  }, [enabled, offlineUserId, persistLatest, projectId]);
  ...
  const queueDraft = useCallback(async (photos: OfflinePhotoInput[] = []) => {
    ...
    if (!hydratedRef.current) throw new Error('The saved draft could not be verified. Keep this page open and try again before queueing.');
