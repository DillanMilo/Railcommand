...
export default function ServiceWorkerProvider({
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data: { session } }) => {
      if (!cancelled) setOfflineUserId(session?.user.id ?? null);
    });

    const { data: authListener } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_OUT") {
        const signedOutUserId = offlineUserIdRef.current;
        if (signedOutUserId) void clearOfflineDataForUser(signedOutUserId);
      }
      setOfflineUserId(session?.user.id ?? null);
    });
  ...
  useEffect(() => {
    ...
    void Promise.all([
      initializeOfflineStorage(offlineUserId),
