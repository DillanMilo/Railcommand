...
import { flushPendingDraftSaves } from '@/lib/offline/pending-draft-saves';
...
export default function OfflineSyncProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      for (const original of queued) {
        ...
        try {
          ...
          // Transport failures can reject instead of returning a server result.
          // Persist the same bounded retry state so queued work stays recoverable.
          result = {
            error: error instanceof Error ? error.message : 'Connection interrupted during synchronization.',
            retryable: true,
          };
        }
  ...
  const requestSignOut = useCallback(async () => {
    ...
    setSignOutBusy(true);
    try {
      // Flush mounted forms before inspecting IndexedDB: their debounce may
      // still hold the user's first edits only in memory.
      await flushPendingDraftSaves(activeUserId);
      ...
      setSignOutDrafts([]);
      setSignOutError(
        'RailCommand could not save or verify your unfinished work. Keep this page open and try again before signing out.'
      );
      setSignOutOpen(true);
    } finally {
      setSignOutBusy(false);
