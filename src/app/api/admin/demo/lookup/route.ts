 ...
 * Does not return demo credentials.
 */
export async function GET(request: NextRequest) {
  ...
  // Get team login display data if team demo
  let teamLogins: { display_name: string; email: string; project_role: string }[] = [];
  if (demo.is_team_demo) {
    const { data: logins } = await admin
      .from('demo_team_logins')
      .select('display_name, email, project_role')
