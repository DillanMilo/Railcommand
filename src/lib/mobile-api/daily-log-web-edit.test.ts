...
function harness(code?:string){
 const calls:any[]=[];let activity=0;
 const query={select:()=>query,eq:()=>query,single:async()=>({data:log,error:null})};
 const db={from:()=>query,rpc:async(name:string,args:any)=>{calls.push({name,args});return {data:{id:'log',project_id:'project'},error:code?{code}:null};}};
 const deps:any={'next/cache':{revalidatePath:()=>{}},'@/lib/supabase/server':{createClient:async()=>db},'@/lib/permissions':{ACTIONS:{}},'./permissions-helper':{getAuthenticatedUser:async()=>({user:{id:'owner'}}),checkPermission:async()=>({allowed:true}),logActivity:async()=>{activity++;}}};
 const exports:any={};runInNewContext(ts.transpileModule(readFileSync(new URL('../actions/daily-logs.ts',import.meta.url),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{exports,require:(name:string)=>deps[name],crypto});
