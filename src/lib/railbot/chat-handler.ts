...
export async function handleRailBotChat(request: NextRequest, mobile?: import('@/lib/mobile-api/auth').MobileAuthenticatedContext) {
  try {
    ...
    for (const msg of messages) {
      if (!msg || typeof msg.content !== 'string' || (mobile && !['user', 'assistant'].includes(msg.role))) {
        return new Response(
          JSON.stringify({ error: 'Invalid message format.' }),
          { status: 400, headers: { 'Content-Type': 'application/json' } },
        );
      }
      if (mobile) { delete msg.tool_calls; delete msg.tool_call_id; }
    ...
    const requestedDemoMode = !mobile && isDemo === true;
    ...
    const rateKey = mobile ? `mobile:${mobile.user.id}` : isDemoMode ? `demo:${ip}` : `auth:${ip}`;
    ...
    if (isDemoMode) {
      ...
      supabase = mobile?.supabase ?? await createClient();
      const {
        data: { user },
        error: authError,
      } = mobile ? { data: { user: mobile.user }, error: null } : await supabase.auth.getUser();
    ...
    if (mobile && incomingConversationId && supabase) {
      const { data: owned } = await supabase.from('conversations').select('id')
        .eq('id', incomingConversationId).eq('user_id', userId).eq('project_id', projectId).maybeSingle();
      if (!owned) return Response.json({ error: 'Conversation not available in this project.' }, { status: 403 });
    }
    ...
    if (isFirstMessage && supabase) {
      ...
      if (mobile && !conversationId) return Response.json({ error: 'Unable to create conversation.' }, { status: 503 });
    }
        ...
        try {
          ...
          const persistMessages = async (finalAssistantContent: string, finalToolCalls?: typeof toolCallsForPersistence) => {
            ...
            try {
              ...
              messagesToInsert.push({
                ...
                tool_calls: finalToolCalls && finalToolCalls.length > 0 ? finalToolCalls.map(t => mobile && ['create_rfi', 'create_punch_list_item', 'create_daily_log'].includes(t.function.name) ? { ...t, mobile_proposal: true } : t) : null,
                model,
                tokens_used: null,
              });

              const saved = await supabase.from('messages').insert(messagesToInsert);
              if (mobile && saved.error) throw new Error('Unable to save conversation. Please retain your message and retry.');
          ...
          while (toolRounds <= MAX_TOOL_ROUNDS) {
            ...
            for (const tc of toolCalls) {
              ...
              if (mobile && ['create_rfi', 'create_punch_list_item', 'create_daily_log'].includes(tc.function.name)) {
                // Persist the proposal before offering confirmation. Never execute an AI write here.
                await persistMessages(assistantContent || 'Review the proposed record before creating it.', [{ id: tc.id, type: 'function', function: tc.function }]);
                send({ type: 'proposal', proposal: { id: tc.id, name: tc.function.name, arguments: parsedArgs } });
                send({ type: 'done' });
                controller.close();
                return;
              }
