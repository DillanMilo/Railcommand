...
export async function enqueueDailyLogCreate(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    transaction.onerror = () => {
      database.close();
      reject(transaction.error ?? new Error('Could not queue the daily log and photos'));
    };
    transaction.onabort = () => {
      database.close();
      reject(transaction.error ?? new Error('Daily-log queue transaction was interrupted'));
    };
  });
