...
export default function InvitationScreen() {
  ...
  useEffect(() => { if (token) void mobileApi.getInvitation(token).then(setInvitation).catch((error) => setMessage(error instanceof Error ? error.message : 'Invitation unavailable.')); }, [token]);
  const accept = async () => {
    if (!token) return;
    setBusy(true);
    try { const result = await mobileApi.acceptInvitation(token); await refresh(result.projectId); router.replace('/(tabs)'); }
  ...
  return <Screen><BrandHeader eyebrow="PROJECT INVITATION" title="You’re invited" />
    <Card>{invitation ? <><SectionTitle>{invitation.projectName}</SectionTitle>
      ...
      <PrimaryButton title="Accept invitation" busy={busy} onPress={() => void accept()} /></> : <Text style={styles.message}>{message}</Text>}</Card>
  </Screen>;
}
