...
begin
  ...
  -- Match the application permission matrix and evaluate it on every retry.
  if not exists (
    select 1
  ...
  -- Serialize concurrent retries for the same user and operation.
  ...
  if found then
    ...
    select
      v_log.id,
      left(trim(item->>'equipment_type'), 300),
    ...
    select
      v_log.id,
      left(trim(item->>'description'), 2000),
    ...
    -- The existing on_daily_log_created trigger records this insertion once.
  end if;
